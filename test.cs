...
// Command-line arguments: run test.dfy --target cs --no-verify
// test.dfy
  ...
  decreases *
{
  ...
  var r1 := Match(expr, ""a"");
  print ""(a|b)*a  \""a\""    => "", r1, ""\n"";
  var r2 := Match(expr, ""ba"");
  print ""(a|b)*a  \""ba\""   => "", r2, ""\n"";
  var r3 := Match(expr, ""ab"");
  print ""(a|b)*a  \""ab\""   => "", r3, ""\n"";
  var r4 := Match(expr, ""abba"");
  print ""(a|b)*a  \""abba\"" => "", r4, ""\n"";
  var r5 := Match(expr, """");
  print ""(a|b)*a  \""\""     => "", r5, ""\n"";
  print ""\n=== Compiled DFA ===\n"";
  ...
  var d1 := DFAMatch(dfa, ""a"", alpha);
  print ""(a|b)*a  \""a\""    => "", d1, ""\n"";
  var d2 := DFAMatch(dfa, ""ba"", alpha);
  print ""(a|b)*a  \""ba\""   => "", d2, ""\n"";
  var d3 := DFAMatch(dfa, ""ab"", alpha);
  print ""(a|b)*a  \""ab\""   => "", d3, ""\n"";
  var d4 := DFAMatch(dfa, ""abba"", alpha);
  print ""(a|b)*a  \""abba\"" => "", d4, ""\n"";
  var d5 := DFAMatch(dfa, """", alpha);
  print ""(a|b)*a  \""\""     => "", d5, ""\n"";
  var expr2 := Comp(a, Plus(a, b));
  print ""\n=== a(a|b) via DFA ===\n"";
  var dfa2 := Compile(expr2, alpha);
  print ""DFA states: "", dfa2.nStates, ""\n"";
  var d6 := DFAMatch(dfa2, ""ab"", alpha);
  print ""a(a|b)  \""ab\"" => "", d6, ""\n"";
  var d7 := DFAMatch(dfa2, ""ba"", alpha);
  print ""a(a|b)  \""ba\"" => "", d7, ""\n"";
  var d8 := DFAMatch(dfa2, ""aa"", alpha);
  print ""a(a|b)  \""aa\"" => "", d8, ""\n"";
  var d9 := DFAMatch(dfa2, ""a"", alpha);
  print ""a(a|b)  \""a\""  => "", d9, ""\n"";
}

ghost predicate WellFormedDFA(dfa: DFA, alphabet: set<char>)
  decreases dfa, alphabet
{
  dfa.nStates > 0 &&
  dfa.start < dfa.nStates &&
  (forall s: int {:trigger s in dfa.accepting} :: 
    s in dfa.accepting ==>
      s < dfa.nStates) &&
  forall st: int, c: char {:trigger (st, c)} :: 
    (0 <= st < dfa.nStates &&
    c in alphabet ==>
      (st, c) in dfa.trans) &&
    (0 <= st < dfa.nStates &&
    c in alphabet ==>
      dfa.trans[(st, c)] < dfa.nStates)
}

method DFAMatch(dfa: DFA, s: seq<char>, alphabet: set<char>)
    returns (accepts: bool)
  requires WellFormedDFA(dfa, alphabet)
  requires forall i: int {:trigger s[i]} :: 0 <= i < |s| ==> s[i] in alphabet
  decreases dfa, s, alphabet
{
  var state := dfa.start;
  for i: int := 0 to |s|
...
method SetToSeq(s: set<char>) returns (sq: seq<char>)
  ensures |sq| == |s|
  ensures forall c: char {:trigger c in sq} {:trigger c in s} :: c in s <==> c in sq
  decreases s
{
  sq := [];
  var remaining := s;
  while |remaining| > 0
    ...
    invariant forall c: char {:trigger c in remaining} {:trigger c in s} {:trigger c in sq} :: c in sq <==> c in s && c !in remaining
  ...
  decreases *
{
  ...
  while !done
    ...
    invariant |states| > 0
    invariant forall i: int {:trigger states[i]} :: (0 <= i < |states| ==> states[i] in stateOf) && (0 <= i < |states| ==> stateOf[states[i]] == i)
    invariant forall expr: Exp<char> {:trigger stateOf[expr]} {:trigger expr in stateOf} :: expr in stateOf ==> stateOf[expr] < |states|
    invariant forall p: (nat, char) {:trigger trans[p]} {:trigger p.0} {:trigger p in trans} :: (p in trans ==> p.0 < |states|) && (p in trans ==> trans[p] < |states|)
    invariant forall i: int, c: char {:trigger (i, c)} {:trigger c in alphabet, expanded[i]} :: 0 <= i < |states| && expanded[i] && c in alphabet ==> (i, c) in trans
    invariant done ==> forall i: int {:trigger expanded[i]} :: 0 <= i < |expanded| ==> expanded[i]
    decreases *
  {
    var found := -1;
    for i: int := 0 to |expanded|
      invariant forall j: int {:trigger expanded[j]} :: 0 <= j < i ==> expanded[j]
      invariant found == -1
    {
      if !expanded[i] {
        found := i;
    ...
    if found == -1 {
      ...
      for ci: int := 0 to |alphaSeq|
        ...
        invariant forall p: (nat, char) {:trigger trans[p]} {:trigger p.0} {:trigger p in trans} :: (p in trans ==> p.0 < |states|) && (p in trans ==> trans[p] < |states|)
        invariant forall i: int, c: char {:trigger (i, c)} {:trigger c in alphabet, expanded[i]} :: 0 <= i < |states| && expanded[i] && c in alphabet ==> (i, c) in trans
        invariant forall j: int {:trigger alphaSeq[j]} :: 0 <= j < ci ==> (sid, alphaSeq[j]) in trans
      {
        var c := alphaSeq[ci];
      ...
      assert forall c: char {:trigger (sid, c)} {:trigger c in alphabet} :: c in alphabet ==> (sid, c) in trans;
      expanded := expanded[sid := true];
    }
  }
  var accepting: set<nat> := {};
  for i: int := 0 to |states|
    invariant forall s: int {:trigger s in accepting} :: s in accepting ==> s < |states|
  {
    if Eps(states[i]) {
...
ghost function Walk<A>(L: Languages.Lang<A>, s: seq<A>): Languages.Lang<A>
  decreases |s|
{
  if |s| == 0 then
    L
  else
    Walk(L.delta(s[0]), s[1..])
}
...
lemma /*{:_inductionTrigger Walk(L2, s), Walk(L1, s)}*/ /*{:_induction L1, L2, s}*/ BisimilarWalk<A(!new)>(L1: Languages.Lang<A>, L2: Languages.Lang<A>, s: seq<A>)
...
lemma /*{:_inductionTrigger Walk(L2, s), Walk(L1, s).eps}*/ /*{:_inductionTrigger Walk(L1, s), Bisimilar(L1, L2)}*/ /*{:_induction L1, L2, s}*/ BisimilarWalkEps<A(!new)>(L1: Languages.Lang<A>, L2: Languages.Lang<A>, s: seq<A>)
  requires Bisimilar(L1, L2)
...
lemma /*{:_inductionTrigger FoldDelta(e, s)}*/ /*{:_inductionTrigger Walk(Operational(e), s)}*/ /*{:_induction e, s}*/ OperationalWalkEps<A(!new)>(e: Exp<A>, s: seq<A>)
...
lemma /*{:_inductionTrigger FoldDelta(e, s)}*/ /*{:_inductionTrigger Matches(e, s)}*/ /*{:_induction e, s}*/ MatchesEquivFoldDelta<A(!new)>(e: Exp<A>, s: seq<A>)
...
function Denotational<A(==)>(e: Exp<A>): Languages.Lang<A>
  decreases e
{
  match e
  case Zero() =>
...
greatest predicate Bisimilar<A(!new)>[nat](L1: Languages.Lang<A>, L2: Languages.Lang<A>)
{
  L1.eps == L2.eps &&
  forall a: A {:trigger L2.delta(a)} {:trigger L1.delta(a)} :: 
    Bisimilar(L1.delta(a), L2.delta(a))
}
/*** (note, what is printed here does not show substitutions of calls to prefix predicates)
predicate {:axiom} Bisimilar#<A(!new)>[_k: nat](L1: Languages.Lang<A>, L2: Languages.Lang<A>)
  decreases _k
{
  L1.eps == L2.eps &&
  forall a: A {:trigger L2.delta(a)} {:trigger L1.delta(a)} :: 
...
greatest lemma BisimilarityIsReflexive<A(!new)>[nat](L: Languages.Lang<A>)
  ensures Bisimilar(L, L)
{
}
/***
lemma {:axiom} /*{:_inductionTrigger Bisimilar#[_k](L, L)}*/ /*{:_induction _k, L}*/ BisimilarityIsReflexive#<A(!new)>[_k: nat](L: Languages.Lang<A>)
  ensures Bisimilar#[_k](L, L)
  decreases _k
{
  if 0 < _k {
  }
...
ghost predicate IsAlgebraHomomorphism<A(!new)>(f: Exp<A> -> Languages.Lang<A>)
{
  forall e: Exp<A> {:trigger IsAlgebraHomomorphismPointwise<A>(f, e)} :: 
    IsAlgebraHomomorphismPointwise(f, e)
}

ghost predicate IsAlgebraHomomorphismPointwise<A(!new)>(f: Exp<A> -> Languages.Lang<A>, e: Exp<A>)
  decreases e
{
  Bisimilar<A>(f(e), match e case Zero() => Languages.Zero() case One() => Languages.One() case Char(a) => Languages.Singleton(a) case Plus(e1, e2) => Languages.Plus(f(e1), f(e2)) case Comp(e1, e2) => Languages.Comp(f(e1), f(e2)) case Star(e1) => Languages.Star(f(e1)))
}
...
function Delta<A(==)>(e: Exp<A>): A -> Exp<A>
  decreases e
{
  (a: A) => match e case Zero() => Zero case One() => Zero case Char(b) => (if a == b then One else Zero) case Plus(e1, e2) => Plus(Delta(e1)(a), Delta(e2)(a)) case Comp(e1, e2) => Plus(Comp(Delta(e1)(a), e2), Comp(if Eps(e1) then One else Zero, Delta(e2)(a))) case Star(e1) => Comp(Delta(e1)(a), Star(e1))
}

function Operational<A(==)>(e: Exp<A>): Languages.Lang<A>
  decreases 1, e
{
  Languages.Alpha(Eps(e), (a: A) => Operational(Delta(e)(a)))
}

ghost predicate IsCoalgebraHomomorphism<A(!new)>(f: Exp<A> -> Languages.Lang<A>)
{
  (forall e: Exp<A> {:trigger Eps(e)} {:trigger f(e)} :: 
    f(e).eps == Eps(e)) &&
  forall e: Exp<A>, a: A {:trigger Delta(e)(a)} {:trigger f(e).delta(a)} :: 
    Bisimilar(f(e).delta(a), f(Delta(e)(a)))
}

lemma OperationalIsCoalgebraHomomorphism<A(!new)>()
  ensures IsCoalgebraHomomorphism<A>(Operational)
{
  forall e: Exp<A>, a: A | true
...
lemma /*{:_inductionTrigger Bisimilar#[j](L1, L2), Bisimilar#[k](L1, L2)}*/ /*{:_induction j, k, L1, L2}*/ BisimilarMonotone<A(!new)>(j: nat, k: nat, L1: Languages.Lang<A>, L2: Languages.Lang<A>)
  requires j <= k
  ...
  decreases j
{
  if j != 0 {
    forall a: A | true
      ensures Bisimilar#[j - 1](L1.delta(a), L2.delta(a))
    {
...
lemma /*{:_inductionTrigger Bisimilar#[k](_default.Plus(L1a, L2a), _default.Plus(L1b, L2b))}*/ /*{:_induction k, L1a, L1b, L2a, L2b}*/ PlusCongruencePointwise<A(!new)>(k: nat, L1a: Languages.Lang<A>, L1b: Languages.Lang<A>, L2a: Languages.Lang<A>, L2b: Languages.Lang<A>)
  ...
  decreases k
{
  if k != 0 {
    forall a: A | true
      ensures Bisimilar#[k - 1](Languages.Plus(L1a, L2a).delta(a), Languages.Plus(L1b, L2b).delta(a))
    {
...
lemma /*{:_inductionTrigger Bisimilar#[k](_default.Comp(L1a, L2a), _default.Comp(L1b, L2b))}*/ /*{:_induction k, L1a, L1b, L2a, L2b}*/ CompCongruencePointwise<A(!new)>(k: nat, L1a: Languages.Lang<A>, L1b: Languages.Lang<A>, L2a: Languages.Lang<A>, L2b: Languages.Lang<A>)
  ...
  decreases k
{
  if k != 0 {
    ...
    forall a: A | true
      ensures Bisimilar#[k - 1](Languages.Comp(L1a, L2a).delta(a), Languages.Comp(L1b, L2b).delta(a))
    {
      BisimilarMonotone(k - 1, k, L2a, L2b);
      CompCongruencePointwise(k - 1, L1a.delta(a), L1b.delta(a), L2a, L2b);
      ghost var cond := if L1a.eps then Languages.One<A>() else Languages.Zero<A>();
      BisimilarityIsReflexive(cond);
      CompCongruencePointwise(k - 1, cond, cond, L2a.delta(a), L2b.delta(a));
      PlusCongruencePointwise(k - 1, Languages.Comp(L1a.delta(a), L2a), Languages.Comp(L1b.delta(a), L2b), Languages.Comp(cond, L2a.delta(a)), Languages.Comp(cond, L2b.delta(a)));
    }
  }
}

lemma /*{:_inductionTrigger Eps(e)}*/ /*{:_inductionTrigger Denotational(e)}*/ /*{:_induction e}*/ DenotationalEps<A(!new)>(e: Exp<A>)
  ensures Denotational(e).eps == Eps(e)
  decreases e
{
}

lemma /*{:_inductionTrigger Delta(e)}*/ /*{:_inductionTrigger Denotational(e).delta}*/ /*{:_induction e}*/ DenotationalDelta<A(!new)>(e: Exp<A>, a: A)
  ...
  decreases e, 1
{
  match e {
    ...
    case {:split false} One() =>
      BisimilarityIsReflexive<A>(Denotational<A>(One).delta(a));
    case {:split false} Char(b) =>
      BisimilarityIsReflexive<A>(Denotational<A>(Char(b)).delta(a));
    case {:split false} Plus(e1, e2) =>
      DenotationalDeltaPlus(e1, e2, a);
    case {:split false} Comp(e1, e2) =>
      DenotationalDeltaComp(e1, e2, a);
    case {:split false} Star(e1) =>
      DenotationalDeltaStar(e1, a);
  }
}

lemma /*{:_inductionTrigger Exp<A>.Plus(e1, e2)}*/ /*{:_induction e1, e2}*/ DenotationalDeltaPlus<A(!new)>(e1: Exp<A>, e2: Exp<A>, a: A)
  ensures Bisimilar<A>(Denotational<A>(Plus(e1, e2)).delta(a), Denotational(Delta<A>(Plus(e1, e2))(a)))
...
lemma /*{:_inductionTrigger Exp<A>.Comp(e1, e2)}*/ /*{:_induction e1, e2}*/ DenotationalDeltaComp<A(!new)>(e1: Exp<A>, e2: Exp<A>, a: A)
  ensures Bisimilar<A>(Denotational<A>(Comp(e1, e2)).delta(a), Denotational(Delta<A>(Comp(e1, e2))(a)))
  decreases Comp(e1, e2), 0
{
  DenotationalEps(e1);
  ghost var D1 := Denotational(e1);
  ghost var D2 := Denotational(e2);
  ghost var D1a := Denotational(Delta(e1)(a));
  ghost var D2a := Denotational(Delta(e2)(a));
  ghost var cond := if Eps(e1) then Languages.One<A>() else Languages.Zero<A>();
...
lemma /*{:_inductionTrigger _default.Comp(L1b, L2), _default.Comp(L1a, L2)}*/ /*{:_induction L1a, L1b, L2}*/ DenotationalDeltaCompPart1<A(!new)>(L1a: Languages.Lang<A>, L1b: Languages.Lang<A>, L2: Languages.Lang<A>)
  requires Bisimilar(L1a, L1b)
...
lemma /*{:_inductionTrigger _default.Comp(cond, L2b), _default.Comp(cond, L2a)}*/ /*{:_induction cond, L2a, L2b}*/ DenotationalDeltaCompPart2<A(!new)>(cond: Languages.Lang<A>, L2a: Languages.Lang<A>, L2b: Languages.Lang<A>)
...
lemma /*{:_inductionTrigger _default.Comp(cond, D2a), _default.Comp(cond, D2da), _default.Comp(D1a, D2), _default.Comp(D1da, D2)}*/ /*{:_induction D1da, D1a, cond, D2, D2da, D2a}*/ DenotationalDeltaCompCombine<A(!new)>(D1da: Languages.Lang<A>, D1a: Languages.Lang<A>, cond: Languages.Lang<A>, D2: Languages.Lang<A>, D2da: Languages.Lang<A>, D2a: Languages.Lang<A>)
  ...
  ensures Bisimilar(Languages.Plus(Languages.Comp(D1da, D2), Languages.Comp(cond, D2da)), Languages.Plus(Languages.Comp(D1a, D2), Languages.Comp(cond, D2a)))
{
  PlusCongruence(Languages.Comp(D1da, D2), Languages.Comp(D1a, D2), Languages.Comp(cond, D2da), Languages.Comp(cond, D2a));
}

lemma /*{:_inductionTrigger Exp<A>.Star(e1)}*/ /*{:_induction e1}*/ DenotationalDeltaStar<A(!new)>(e1: Exp<A>, a: A)
  ensures Bisimilar<A>(Denotational<A>(Star(e1)).delta(a), Denotational(Delta<A>(Star(e1))(a)))
  decreases Star(e1), 0
{
  DenotationalDelta(e1, a);
...
{
  forall e: Exp<A> | true
    ensures Denotational<A>(e).eps == Eps<A>(e)
  {
    DenotationalEps(e);
  }
  forall e: Exp<A>, a: A | true
    ensures Bisimilar<A>(Denotational<A>(e).delta(a), Denotational(Delta<A>(e)(a)))
  {
    DenotationalDelta(e, a);
  }
}

greatest lemma PlusCongruence<A(!new)>[nat](L1a: Languages.Lang<A>, L1b: Languages.Lang<A>, L2a: Languages.Lang<A>, L2b: Languages.Lang<A>)
  requires Bisimilar(L1a, L1b)
  requires Bisimilar(L2a, L2b)
...
/***
lemma {:axiom} /*{:_inductionTrigger Bisimilar#[_k](_default.Plus(L1a, L2a), _default.Plus(L1b, L2b))}*/ /*{:_induction _k, L1a, L1b, L2a, L2b}*/ PlusCongruence#<A(!new)>[_k: nat](L1a: Languages.Lang<A>, L1b: Languages.Lang<A>, L2a: Languages.Lang<A>, L2b: Languages.Lang<A>)
  requires Bisimilar(L1a, L1b)
  requires Bisimilar(L2a, L2b)
  ensures Bisimilar#[_k](Languages.Plus(L1a, L2a), Languages.Plus(L1b, L2b))
  decreases _k
{
  if 0 < _k {
  }
...
lemma /*{:_inductionTrigger _default.Comp(L1b, L2b), _default.Comp(L1a, L2a)}*/ /*{:_induction L1a, L1b, L2a, L2b}*/ CompCongruence<A(!new)>(L1a: Languages.Lang<A>, L1b: Languages.Lang<A>, L2a: Languages.Lang<A>, L2b: Languages.Lang<A>)
  requires Bisimilar(L1a, L1b)
  requires Bisimilar(L2a, L2b)
...
lemma UniqueCoalgebraHomomorphism<A(!new)>(f: Exp<A> -> Languages.Lang<A>, g: Exp<A> -> Languages.Lang<A>, e: Exp<A>)
...
greatest lemma BisimilarityIsTransitive<A(!new)>[nat](L1: Languages.Lang<A>, L2: Languages.Lang<A>, L3: Languages.Lang<A>)
  requires Bisimilar(L1, L2) && Bisimilar(L2, L3)
  ensures Bisimilar(L1, L3)
{
}
/***
lemma {:axiom} /*{:_inductionTrigger Bisimilar#[_k](L1, L3), Bisimilar(L2, L3)}*/ /*{:_inductionTrigger Bisimilar#[_k](L1, L3), Bisimilar(L1, L2)}*/ /*{:_induction _k, L1, L2, L3}*/ BisimilarityIsTransitive#<A(!new)>[_k: nat](L1: Languages.Lang<A>, L2: Languages.Lang<A>, L3: Languages.Lang<A>)
  requires Bisimilar(L1, L2) && Bisimilar(L2, L3)
  ensures Bisimilar#[_k](L1, L3)
  decreases _k
{
  if 0 < _k {
  }
...
lemma /*{:_inductionTrigger Bisimilar#[k](L1, L2)}*/ /*{:_induction k, L1, L2}*/ UniqueCoalgebraHomomorphismHelperPointwise<A(!new)>(k: nat, f: Exp<A> -> Languages.Lang<A>, g: Exp<A> -> Languages.Lang<A>, L1: Languages.Lang<A>, L2: Languages.Lang<A>)
  requires IsCoalgebraHomomorphism(f)
  requires IsCoalgebraHomomorphism(g)
  requires exists e: Exp<A> {:trigger g(e)} {:trigger f(e)} :: Bisimilar#[k](L1, f(e)) && Bisimilar#[k](L2, g(e))
  ensures Bisimilar#[k](L1, L2)
  decreases k
{
  ghost var e :| Bisimilar#[k](L1, f(e)) && Bisimilar#[k](L2, g(e));
  if k != 0 {
    forall a: A | true
...
lemma /*{:_inductionTrigger Bisimilar#[k](L1, L3), Bisimilar#[k](L2, L3)}*/ /*{:_inductionTrigger Bisimilar#[k](L2, L3), Bisimilar#[k](L1, L2)}*/ /*{:_induction k, L1, L2, L3}*/ BisimilarityIsTransitivePointwise<A(!new)>(k: nat, L1: Languages.Lang<A>, L2: Languages.Lang<A>, L3: Languages.Lang<A>)
...
lemma /*{:_inductionTrigger Denotational(e)}*/ /*{:_inductionTrigger Operational(e)}*/ /*{:_induction e}*/ OperationalAndDenotationalAreBisimilar<A(!new)>(e: Exp<A>)
...
greatest lemma BisimilarityIsSymmetric<A(!new)>[nat](L1: Languages.Lang<A>, L2: Languages.Lang<A>)
  ensures Bisimilar(L1, L2) ==> Bisimilar(L2, L1)
  ensures Bisimilar(L1, L2) <== Bisimilar(L2, L1)
{
}
/***
lemma {:axiom} /*{:_inductionTrigger Bisimilar#[_k](L1, L2)}*/ /*{:_inductionTrigger Bisimilar#[_k](L2, L1)}*/ /*{:_induction _k, L1, L2}*/ BisimilarityIsSymmetric#<A(!new)>[_k: nat](L1: Languages.Lang<A>, L2: Languages.Lang<A>)
  ensures Bisimilar(L1, L2) ==> Bisimilar#[_k](L2, L1)
  ensures Bisimilar#[_k](L1, L2) <== Bisimilar(L2, L1)
  decreases _k
{
  if 0 < _k {
  }
...
lemma /*{:_inductionTrigger Bisimilar#[k](_default.Star(L1), _default.Star(L2))}*/ /*{:_induction k, L1, L2}*/ StarCongruencePointwise<A(!new)>(k: nat, L1: Languages.Lang<A>, L2: Languages.Lang<A>)
  requires Bisimilar(L1, L2)
  ...
  decreases k
{
  if k != 0 {
    forall a: A | true
      ensures Bisimilar#[k - 1](Languages.Star(L1).delta(a), Languages.Star(L2).delta(a))
    {
      StarCongruencePointwise(k - 1, L1, L2);
      CompCongruencePointwise(k - 1, L1.delta(a), L2.delta(a), Languages.Star(L1), Languages.Star(L2));
    }
  }
}

lemma /*{:_inductionTrigger _default.Star(L2), _default.Star(L1)}*/ /*{:_induction L1, L2}*/ StarCongruence<A(!new)>(L1: Languages.Lang<A>, L2: Languages.Lang<A>)
  requires Bisimilar(L1, L2)
...
lemma OperationalIsAlgebraHomomorphismPointwise<A(!new)>(e: Exp<A>)
  ensures IsAlgebraHomomorphismPointwise<A>(Operational, e)
  decreases e
{
  ...
  match e {
    case {:split false} Zero() =>
      ...
      BisimilarityIsTransitive(Operational<A>(Zero), Denotational<A>(Zero), Languages.Zero<A>());
    case {:split false} One() =>
      BisimilarityIsReflexive<A>(Languages.One<A>());
      BisimilarityIsTransitive(Operational<A>(One), Denotational<A>(One), Languages.One<A>());
    case {:split false} Char(c) =>
      BisimilarityIsReflexive<A>(Languages.Singleton(c));
      BisimilarityIsTransitive(Operational(Char(c)), Denotational(Char(c)), Languages.Singleton(c));
    case {:split false} Plus(e1, e2) =>
      ...
      PlusCongruence(Denotational(e1), Operational(e1), Denotational(e2), Operational(e2));
      BisimilarityIsTransitive(Operational(Plus(e1, e2)), Denotational(Plus(e1, e2)), Languages.Plus(Operational(e1), Operational(e2)));
    case {:split false} Comp(e1, e2) =>
      OperationalAndDenotationalAreBisimilar(e1);
      OperationalAndDenotationalAreBisimilar(e2);
      BisimilarityIsSymmetric(Operational(e1), Denotational(e1));
      BisimilarityIsSymmetric(Operational(e2), Denotational(e2));
      CompCongruence(Denotational(e1), Operational(e1), Denotational(e2), Operational(e2));
      BisimilarityIsTransitive(Operational(Comp(e1, e2)), Denotational(Comp(e1, e2)), Languages.Comp(Operational(e1), Operational(e2)));
    case {:split false} Star(e1) =>
      OperationalAndDenotationalAreBisimilar(e1);
      BisimilarityIsSymmetric(Operational(e1), Denotational(e1));
      StarCongruence(Denotational(e1), Operational(e1));
      BisimilarityIsTransitive(Operational(Star(e1)), Denotational(Star(e1)), Languages.Star(Operational(e1)));
  }
}
  ...
  decreases e
{
  ...
  case Char(a) =>
    Char(a)
  case Plus(e1, e2) =>
    NormPlus(Normalize(e1), Normalize(e2))
  case Comp(e1, e2) =>
    NormComp(Normalize(e1), Normalize(e2))
  case Star(e1) =>
    NormStar(Normalize(e1))
}
  ...
  decreases |s|
{
  if |s| == 0 then
    ...
    FoldNDelta(NDelta(e)(s[0]), s[1..])
}

lemma /*{:_inductionTrigger Bisimilar#[k](_default.Plus(X, Y), Z)}*/ /*{:_induction k, X, Y, Z}*/ PlusBisimRightZero<A(!new)>(k: nat, X: Languages.Lang<A>, Y: Languages.Lang<A>, Z: Languages.Lang<A>)
  ...
  decreases k
{
  if k != 0 {
    forall a: A | true
      ensures Bisimilar#[k - 1](Languages.Plus(X, Y).delta(a), Z.delta(a))
    {
...
lemma /*{:_inductionTrigger Bisimilar#[k](_default.Plus(X, Y), Z)}*/ /*{:_induction k, X, Y, Z}*/ PlusBisimLeftZero<A(!new)>(k: nat, X: Languages.Lang<A>, Y: Languages.Lang<A>, Z: Languages.Lang<A>)
...
lemma /*{:_inductionTrigger Bisimilar#[k](_default.Plus(X, Y), _default.Zero())}*/ /*{:_induction k, X, Y}*/ PlusBisimBothZero<A(!new)>(k: nat, X: Languages.Lang<A>, Y: Languages.Lang<A>)
  requires Bisimilar#[k](X, Languages.Zero<A>())
  requires Bisimilar#[k](Y, Languages.Zero<A>())
...
greatest lemma PlusZeroRight<A(!new)>[nat](L: Languages.Lang<A>)
  ensures Bisimilar(Languages.Plus(L, Languages.Zero()), L)
{
}
/***
lemma {:axiom} /*{:_inductionTrigger Bisimilar#[_k](_default.Plus(L, _default.Zero()), L)}*/ /*{:_induction _k, L}*/ PlusZeroRight#<A(!new)>[_k: nat](L: Languages.Lang<A>)
  ensures Bisimilar#[_k](Languages.Plus(L, Languages.Zero()), L)
  decreases _k
{
  if 0 < _k {
  }
...
greatest lemma PlusZeroLeft<A(!new)>[nat](L: Languages.Lang<A>)
  ensures Bisimilar(Languages.Plus(Languages.Zero(), L), L)
{
}
/***
lemma {:axiom} /*{:_inductionTrigger Bisimilar#[_k](_default.Plus(_default.Zero(), L), L)}*/ /*{:_induction _k, L}*/ PlusZeroLeft#<A(!new)>[_k: nat](L: Languages.Lang<A>)
  ensures Bisimilar#[_k](Languages.Plus(Languages.Zero(), L), L)
  decreases _k
{
  if 0 < _k {
  }
...
greatest lemma PlusIdem<A(!new)>[nat](L: Languages.Lang<A>)
  ensures Bisimilar(Languages.Plus(L, L), L)
{
}
/***
lemma {:axiom} /*{:_inductionTrigger Bisimilar#[_k](_default.Plus(L, L), L)}*/ /*{:_induction _k, L}*/ PlusIdem#<A(!new)>[_k: nat](L: Languages.Lang<A>)
  ensures Bisimilar#[_k](Languages.Plus(L, L), L)
  decreases _k
{
  if 0 < _k {
  }
...
lemma /*{:_inductionTrigger Bisimilar#[k](_default.Comp(_default.Zero(), L), _default.Zero())}*/ /*{:_induction k, L}*/ CompZeroLeftPrefix<A(!new)>(k: nat, L: Languages.Lang<A>)
  ensures Bisimilar#[k](Languages.Comp(Languages.Zero(), L), Languages.Zero<A>())
  decreases k
{
  if k != 0 {
    forall a: A | true
      ensures Bisimilar#[k - 1](Languages.Comp(Languages.Zero(), L).delta(a), Languages.Zero<A>().delta(a))
    {
      CompZeroLeftPrefix(k - 1, L);
      CompZeroLeftPrefix(k - 1, L.delta(a));
      PlusBisimBothZero(k - 1, Languages.Comp(Languages.Zero<A>(), L), Languages.Comp(Languages.Zero<A>(), L.delta(a)));
    }
  }
}

lemma /*{:_inductionTrigger _default.Comp(_default.Zero(), L)}*/ /*{:_induction L}*/ CompZeroLeft<A(!new)>(L: Languages.Lang<A>)
  ensures Bisimilar(Languages.Comp(Languages.Zero(), L), Languages.Zero<A>())
{
  forall k: nat | true {
    CompZeroLeftPrefix(k, L);
  }
}

lemma {:isolate_assertions} /*{:_inductionTrigger Bisimilar#[k](_default.Comp(L, _default.Zero()), _default.Zero())}*/ /*{:_induction k, L}*/ CompZeroRightPrefix<A(!new)>(k: nat, L: Languages.Lang<A>)
  ensures Bisimilar#[k](Languages.Comp(L, Languages.Zero()), Languages.Zero<A>())
  decreases k
{
  if k != 0 {
    ghost var Z := Languages.Zero<A>();
    ghost var cond := if L.eps then Languages.One<A>() else Languages.Zero<A>();
    forall a: A | true
      ensures Bisimilar#[k - 1](Languages.Comp(L, Z).delta(a), Z.delta(a))
    {
      CompZeroRightPrefix(k - 1, L.delta(a));
      CompZeroRightPrefix(k - 1, cond);
      PlusBisimBothZero(k - 1, Languages.Comp(L.delta(a), Z), Languages.Comp(cond, Z));
    }
  }
}

lemma /*{:_inductionTrigger _default.Comp(L, _default.Zero())}*/ /*{:_induction L}*/ CompZeroRight<A(!new)>(L: Languages.Lang<A>)
  ensures Bisimilar(Languages.Comp(L, Languages.Zero()), Languages.Zero<A>())
{
  forall k: nat | true {
    CompZeroRightPrefix(k, L);
  }
}

lemma /*{:_inductionTrigger Bisimilar#[k](_default.Comp(_default.One(), L), L)}*/ /*{:_induction k, L}*/ CompOneLeftPrefix<A(!new)>(k: nat, L: Languages.Lang<A>)
  ensures Bisimilar#[k](Languages.Comp(Languages.One(), L), L)
  decreases k
{
  if k != 0 {
    forall a: A | true
      ensures Bisimilar#[k - 1](Languages.Comp(Languages.One(), L).delta(a), L.delta(a))
    {
      CompZeroLeftPrefix(k - 1, L);
      CompOneLeftPrefix(k - 1, L.delta(a));
      PlusBisimLeftZero(k - 1, Languages.Comp(Languages.Zero<A>(), L), Languages.Comp(Languages.One(), L.delta(a)), L.delta(a));
    }
  }
}

lemma /*{:_inductionTrigger _default.Comp(_default.One(), L)}*/ /*{:_induction L}*/ CompOneLeft<A(!new)>(L: Languages.Lang<A>)
  ensures Bisimilar(Languages.Comp(Languages.One(), L), L)
{
  forall k: nat | true {
    CompOneLeftPrefix(k, L);
  }
}
...
{
  forall a: A | true
    ensures Bisimilar(Languages.Star(Languages.Zero<A>()).delta(a), Languages.One<A>().delta(a))
  {
    CompZeroLeft(Languages.Star(Languages.Zero<A>()));
  }
}
/***
lemma {:axiom} /*{:_inductionTrigger Bisimilar#[_k](_default.Star(_default.Zero()), _default.One())}*/ /*{:_induction _k}*/ StarZero#<A(!new)>[_k: nat]()
  ensures Bisimilar#<A>[_k](Languages.Star(Languages.Zero()), Languages.One())
  decreases _k
{
  if 0 < _k {
    forall a: A | true
      ensures Bisimilar#[_k - 1](Languages.Star(Languages.Zero<A>()).delta(a), Languages.One<A>().delta(a))
    {
      CompZeroLeft(Languages.Star(Languages.Zero<A>()));
    }
  }
...
{
  forall a: A | true
    ensures Bisimilar(Languages.Star(Languages.One<A>()).delta(a), Languages.One<A>().delta(a))
  {
    CompZeroLeft(Languages.Star(Languages.One<A>()));
  }
}
/***
lemma {:axiom} /*{:_inductionTrigger Bisimilar#[_k](_default.Star(_default.One()), _default.One())}*/ /*{:_induction _k}*/ StarOne#<A(!new)>[_k: nat]()
  ensures Bisimilar#<A>[_k](Languages.Star(Languages.One()), Languages.One())
  decreases _k
{
  if 0 < _k {
    forall a: A | true
      ensures Bisimilar#[_k - 1](Languages.Star(Languages.One<A>()).delta(a), Languages.One<A>().delta(a))
    {
      CompZeroLeft(Languages.Star(Languages.One<A>()));
    }
  }
...
lemma /*{:_inductionTrigger _default.Plus(Denotational(e1), Denotational(e2))}*/ /*{:_inductionTrigger NormPlus(e1, e2)}*/ /*{:_induction e1, e2}*/ NormPlusCorrect<A(!new)>(e1: Exp<A>, e2: Exp<A>)
  ensures Bisimilar<A>(Denotational(NormPlus(e1, e2)), Languages.Plus(Denotational(e1), Denotational(e2)))
  decreases e1, e2
{
  if e1 == Zero {
    PlusZeroLeft(Denotational(e2));
    BisimilarityIsSymmetric(Languages.Plus(Denotational<A>(e1), Denotational(e2)), Denotational(e2));
  } else if e2 == Zero {
    PlusZeroRight(Denotational(e1));
    BisimilarityIsSymmetric(Languages.Plus(Denotational<A>(e1), Denotational(e2)), Denotational(e1));
  } else if e1 == e2 {
    PlusIdem(Denotational(e1));
    BisimilarityIsSymmetric(Languages.Plus(Denotational<A>(e1), Denotational(e1)), Denotational(e1));
  } else {
    BisimilarityIsReflexive(Denotational(Plus(e1, e2)));
  }
}

lemma /*{:_inductionTrigger _default.Comp(Denotational(e1), Denotational(e2))}*/ /*{:_inductionTrigger NormComp(e1, e2)}*/ /*{:_induction e1, e2}*/ NormCompCorrect<A(!new)>(e1: Exp<A>, e2: Exp<A>)
  ensures Bisimilar<A>(Denotational(NormComp(e1, e2)), Languages.Comp(Denotational(e1), Denotational(e2)))
  decreases e1, e2
{
  if e1 == Zero {
    CompZeroLeft(Denotational(e2));
    BisimilarityIsSymmetric(Languages.Comp(Denotational<A>(e1), Denotational(e2)), Languages.Zero());
  } else if e2 == Zero {
    CompZeroRight(Denotational(e1));
    BisimilarityIsSymmetric(Languages.Comp(Denotational<A>(e1), Denotational(e2)), Languages.Zero());
  } else if e1 == One {
    CompOneLeft(Denotational(e2));
    BisimilarityIsSymmetric(Languages.Comp(Denotational<A>(e1), Denotational(e2)), Denotational(e2));
  } else {
    BisimilarityIsReflexive(Denotational(Comp(e1, e2)));
  }
}

lemma /*{:_inductionTrigger _default.Star(Denotational(e))}*/ /*{:_inductionTrigger NormStar(e)}*/ /*{:_induction e}*/ NormStarCorrect<A(!new)>(e: Exp<A>)
  ensures Bisimilar<A>(Denotational(NormStar(e)), Languages.Star(Denotational(e)))
  decreases e
{
  match e {
    case {:split false} Zero() =>
      StarZero<A>();
      BisimilarityIsSymmetric(Languages.Star(Denotational<A>(Zero)), Languages.One<A>());
    case {:split false} One() =>
      StarOne<A>();
      BisimilarityIsSymmetric(Languages.Star(Denotational<A>(One)), Languages.One<A>());
    case {:split false} _ /* _v1 */ =>
      BisimilarityIsReflexive(Denotational(Star(e)));
  }
}

lemma /*{:_inductionTrigger Normalize(e)}*/ /*{:_induction e}*/ NormalizeCorrect<A(!new)>(e: Exp<A>)
  ensures Bisimilar<A>(Denotational(Normalize(e)), Denotational(e))
  decreases e
{
  match e {
    case {:split false} Zero() =>
      BisimilarityIsReflexive<A>(Denotational<A>(Zero));
    case {:split false} One() =>
      BisimilarityIsReflexive<A>(Denotational<A>(One));
    case {:split false} Char(a) =>
      BisimilarityIsReflexive<A>(Denotational(Char(a)));
    case {:split false} Plus(e1, e2) =>
      NormalizeCorrect(e1);
      NormalizeCorrect(e2);
      PlusCongruence(Denotational(Normalize(e1)), Denotational(e1), Denotational(Normalize(e2)), Denotational(e2));
      NormPlusCorrect<A>(Normalize(e1), Normalize(e2));
      BisimilarityIsTransitive(Denotational(Normalize(Plus(e1, e2))), Languages.Plus(Denotational(Normalize(e1)), Denotational(Normalize(e2))), Languages.Plus(Denotational(e1), Denotational(e2)));
    case {:split false} Comp(e1, e2) =>
      NormalizeCorrect(e1);
      NormalizeCorrect(e2);
      CompCongruence(Denotational(Normalize(e1)), Denotational(e1), Denotational(Normalize(e2)), Denotational(e2));
      NormCompCorrect<A>(Normalize(e1), Normalize(e2));
      BisimilarityIsTransitive(Denotational(Normalize(Comp(e1, e2))), Languages.Comp(Denotational(Normalize(e1)), Denotational(Normalize(e2))), Languages.Comp(Denotational(e1), Denotational(e2)));
    case {:split false} Star(e1) =>
      NormalizeCorrect(e1);
      StarCongruence(Denotational(Normalize(e1)), Denotational(e1));
      NormStarCorrect<A>(Normalize(e1));
      BisimilarityIsTransitive(Denotational(Normalize(Star(e1))), Languages.Star(Denotational(Normalize(e1))), Languages.Star(Denotational(e1)));
  }
}
...
lemma /*{:_inductionTrigger Delta(FoldDelta(e, s))}*/ /*{:_inductionTrigger FoldDelta(e, s + [a])}*/ /*{:_induction e, s}*/ FoldDeltaAppend<A(!new)>(e: Exp<A>, s: seq<A>, a: A)
...
datatype DFA = DFA(nStates: nat, start: nat, accepting: set<nat>, trans: map<(nat, char), nat>)

datatype Exp<A> = Zero | One | Char(A) | Plus(Exp<A>, Exp<A>) | Comp(Exp<A>, Exp<A>) | Star(Exp<A>)

codatatype Lang<!A> = Alpha(eps: bool, delta: A -> Lang<A>)

module Languages {
  function Zero<A>(): Lang<A>
    decreases 1
  {
    Alpha(false, (a: A) => Zero())
  }

  function One<A>(): Lang<A>
  {
    Alpha(true, (a: A) => Zero())
  }

  function Singleton<A(==)>(a: A): Lang<A>
  {
    Alpha(false, (b: A) => if a == b then One() else Zero())
  }

  function {:abstemious} Plus<A>(L1: Lang<A>, L2: Lang<A>): Lang<A>
    decreases 1
  {
    Alpha(L1.eps || L2.eps, (a: A) => Plus(L1.delta(a), L2.delta(a)))
  }

  function {:abstemious} Comp<A>(L1: Lang<A>, L2: Lang<A>): Lang<A>
    decreases 1
  {
    Alpha(L1.eps && L2.eps, (a: A) => Plus(Comp(L1.delta(a), L2), Comp(if L1.eps then One() else Zero(), L2.delta(a))))
  }

  function {:abstemious} Star<A>(L: Lang<A>): Lang<A>
    decreases 1
  {
...
namespace Languages {
  ...
  public partial class __default {
    public static Languages._ILang<__A> Zero<__A>() {
      return new Languages.Lang__Lazy<__A>(() => { return new Languages.Lang_Alpha<__A>(false, ((System.Func<__A, Languages._ILang<__A>>)((_0_a) => {
  return Languages.__default.Zero<__A>();
}))); });
    }
    public static Languages._ILang<__A> One<__A>() {
      return Languages.Lang<__A>.create_Alpha(true, ((System.Func<__A, Languages._ILang<__A>>)((_0_a) => {
  return Languages.__default.Zero<__A>();
})));
    }
    public static Languages._ILang<__A> Singleton<__A>(__A a) {
      return Languages.Lang<__A>.create_Alpha(false, Dafny.Helpers.Id<Func<__A, Func<__A, Languages._ILang<__A>>>>((_0_a) => ((System.Func<__A, Languages._ILang<__A>>)((_1_b) => {
  return ((object.Equals(_0_a, _1_b)) ? (Languages.__default.One<__A>()) : (Languages.__default.Zero<__A>()));
})))(a));
    }
    public static Languages._ILang<__A> Plus<__A>(Languages._ILang<__A> L1, Languages._ILang<__A> L2)
    {
      return new Languages.Lang__Lazy<__A>(() => { return new Languages.Lang_Alpha<__A>(((L1).dtor_eps) || ((L2).dtor_eps), Dafny.Helpers.Id<Func<Languages._ILang<__A>, Languages._ILang<__A>, Func<__A, Languages._ILang<__A>>>>((_0_L1, _1_L2) => ((System.Func<__A, Languages._ILang<__A>>)((_2_a) => {
  return Languages.__default.Plus<__A>(Dafny.Helpers.Id<Func<__A, Languages._ILang<__A>>>((_0_L1).dtor_delta)(_2_a), Dafny.Helpers.Id<Func<__A, Languages._ILang<__A>>>((_1_L2).dtor_delta)(_2_a));
})))(L1, L2)); });
    }
    public static Languages._ILang<__A> Comp<__A>(Languages._ILang<__A> L1, Languages._ILang<__A> L2)
    {
      return new Languages.Lang__Lazy<__A>(() => { return new Languages.Lang_Alpha<__A>(((L1).dtor_eps) && ((L2).dtor_eps), Dafny.Helpers.Id<Func<Languages._ILang<__A>, Languages._ILang<__A>, Func<__A, Languages._ILang<__A>>>>((_0_L1, _1_L2) => ((System.Func<__A, Languages._ILang<__A>>)((_2_a) => {
  return Languages.__default.Plus<__A>(Languages.__default.Comp<__A>(Dafny.Helpers.Id<Func<__A, Languages._ILang<__A>>>((_0_L1).dtor_delta)(_2_a), _1_L2), Languages.__default.Comp<__A>((((_0_L1).dtor_eps) ? (Languages.__default.One<__A>()) : (Languages.__default.Zero<__A>())), Dafny.Helpers.Id<Func<__A, Languages._ILang<__A>>>((_1_L2).dtor_delta)(_2_a)));
})))(L1, L2)); });
    }
    public static Languages._ILang<__A> Star<__A>(Languages._ILang<__A> L) {
      return new Languages.Lang__Lazy<__A>(() => { return new Languages.Lang_Alpha<__A>(true, Dafny.Helpers.Id<Func<Languages._ILang<__A>, Func<__A, Languages._ILang<__A>>>>((_0_L) => ((System.Func<__A, Languages._ILang<__A>>)((_1_a) => {
  return Languages.__default.Comp<__A>(Dafny.Helpers.Id<Func<__A, Languages._ILang<__A>>>((_0_L).dtor_delta)(_1_a), Languages.__default.Star<__A>(_0_L));
})))(L)); });
    }
  }

  public interface _ILang<A> {
    _ILang<A> _Get();
    bool is_Alpha { get; }
    bool dtor_eps { get; }
    Func<A, Languages._ILang<A>> dtor_delta { get; }
    _ILang<__A> DowncastClone<__A>(Func<A, __A> converter0);
  }
  public abstract class Lang<A> : _ILang<A> {
    public Lang() {
    }
    public static Languages._ILang<A> Default(A _default_A) {
      return new Languages.Lang__Lazy<A>(() => { return create_Alpha(false, ((A x0) => Languages.Lang<A>.Default(_default_A))); });
    }
    public static Dafny.TypeDescriptor<Languages._ILang<A>> _TypeDescriptor(Dafny.TypeDescriptor<A> _td_A) {
      return new Dafny.TypeDescriptor<Languages._ILang<A>>(Languages.Lang<A>.Default(_td_A.Default()));
    }
    public abstract _ILang<A> _Get();
    public static _ILang<A> create_Alpha(bool eps, Func<A, Languages._ILang<A>> delta) {
      return new Lang_Alpha<A>(eps, delta);
    }
    public bool is_Alpha { get { return this is Lang_Alpha<A>; } }
    public bool dtor_eps {
      get {
        var d = this._Get();
        return ((Lang_Alpha<A>)d)._eps;
      }
    }
    public Func<A, Languages._ILang<A>> dtor_delta {
      get {
        var d = this._Get();
        return ((Lang_Alpha<A>)d)._delta;
      }
    }
    public abstract _ILang<__A> DowncastClone<__A>(Func<A, __A> converter0);
  }
  public class Lang__Lazy<A> : Lang<A> {
    public delegate _ILang<A> Computer();
    Computer c;
    _ILang<A> d;
    public Lang__Lazy(Computer c) : base() {
      this.c = c;
    }
    public override _ILang<__A> DowncastClone<__A>(Func<A, __A> converter0) {
      if (this is _ILang<__A> dt) { return dt; }
      return new Languages.Lang__Lazy<__A>(() => _Get().DowncastClone<__A>(converter0));
    }
    public override _ILang<A> _Get() { if (c != null) { d = c(); c = null; } return d; }
    public override string ToString() { return _Get().ToString(); }
  }
  public class Lang_Alpha<A> : Lang<A> {
    public readonly bool _eps;
    public readonly Func<A, Languages._ILang<A>> _delta;
    public Lang_Alpha(bool eps, Func<A, Languages._ILang<A>> delta) : base() {
      this._eps = eps;
      this._delta = delta;
    }
    public override _ILang<A> _Get() { return this; }
    public override _ILang<__A> DowncastClone<__A>(Func<A, __A> converter0) {
      if (this is _ILang<__A> dt) { return dt; }
      return new Lang_Alpha<__A>(_eps, (_delta).DowncastClone<A, Languages._ILang<A>, __A, Languages._ILang<__A>>(Dafny.Helpers.CastConverter<__A, A>, Dafny.Helpers.CastConverter<Languages._ILang<A>, Languages._ILang<__A>>));
    }
    public override bool Equals(object other) {
      var oth = other as Languages.Lang_Alpha<A>;
      return oth != null && this._eps == oth._eps && object.Equals(this._delta, oth._delta);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._eps));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._delta));
      return (int) hash;
    }
    public override string ToString() {
      return "Languages.Lang.Alpha";
    }
  }
} // end of namespace Languages
namespace _module {

  public partial class __default {
    public static void _Main(Dafny.ISequence<Dafny.ISequence<Dafny.Rune>> __noArgsParameter)
    {
      _IExp<Dafny.Rune> _0_a;
      _0_a = _module.Exp<Dafny.Rune>.create_Char(new Dafny.Rune('a'));
      _IExp<Dafny.Rune> _1_b;
      _1_b = _module.Exp<Dafny.Rune>.create_Char(new Dafny.Rune('b'));
      _IExp<Dafny.Rune> _2_ab__star;
      _2_ab__star = _module.Exp<Dafny.Rune>.create_Star(_module.Exp<Dafny.Rune>.create_Plus(_0_a, _1_b));
      _IExp<Dafny.Rune> _3_expr;
      _3_expr = _module.Exp<Dafny.Rune>.create_Comp(_2_ab__star, _0_a);
      Dafny.ISet<Dafny.Rune> _4_alpha;
      _4_alpha = Dafny.Set<Dafny.Rune>.FromElements(new Dafny.Rune('a'), new Dafny.Rune('b'));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("=== Derivative matcher ===\n")).ToVerbatimString(false));
      bool _5_r1;
      bool _out0;
      _out0 = __default.Match(_3_expr, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("a"));
      _5_r1 = _out0;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(a|b)*a  \"a\"    => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_5_r1));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _6_r2;
      bool _out1;
      _out1 = __default.Match(_3_expr, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("ba"));
      _6_r2 = _out1;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(a|b)*a  \"ba\"   => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_6_r2));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _7_r3;
      bool _out2;
      _out2 = __default.Match(_3_expr, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("ab"));
      _7_r3 = _out2;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(a|b)*a  \"ab\"   => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_7_r3));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _8_r4;
      bool _out3;
      _out3 = __default.Match(_3_expr, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("abba"));
      _8_r4 = _out3;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(a|b)*a  \"abba\" => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_8_r4));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _9_r5;
      bool _out4;
      _out4 = __default.Match(_3_expr, Dafny.Sequence<Dafny.Rune>.UnicodeFromString(""));
      _9_r5 = _out4;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(a|b)*a  \"\"     => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_9_r5));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n=== Compiled DFA ===\n")).ToVerbatimString(false));
      _IDFA _10_dfa;
      _IDFA _out5;
      _out5 = __default.Compile(_3_expr, _4_alpha);
      _10_dfa = _out5;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("DFA states: ")).ToVerbatimString(false));
      Dafny.Helpers.Print(((_10_dfa).dtor_nStates));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _11_d1;
      bool _out6;
      _out6 = __default.DFAMatch(_10_dfa, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("a"), _4_alpha);
      _11_d1 = _out6;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(a|b)*a  \"a\"    => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_11_d1));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _12_d2;
      bool _out7;
      _out7 = __default.DFAMatch(_10_dfa, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("ba"), _4_alpha);
      _12_d2 = _out7;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(a|b)*a  \"ba\"   => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_12_d2));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _13_d3;
      bool _out8;
      _out8 = __default.DFAMatch(_10_dfa, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("ab"), _4_alpha);
      _13_d3 = _out8;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(a|b)*a  \"ab\"   => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_13_d3));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _14_d4;
      bool _out9;
      _out9 = __default.DFAMatch(_10_dfa, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("abba"), _4_alpha);
      _14_d4 = _out9;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(a|b)*a  \"abba\" => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_14_d4));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _15_d5;
      bool _out10;
      _out10 = __default.DFAMatch(_10_dfa, Dafny.Sequence<Dafny.Rune>.UnicodeFromString(""), _4_alpha);
      _15_d5 = _out10;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("(a|b)*a  \"\"     => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_15_d5));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      _IExp<Dafny.Rune> _16_expr2;
      _16_expr2 = _module.Exp<Dafny.Rune>.create_Comp(_0_a, _module.Exp<Dafny.Rune>.create_Plus(_0_a, _1_b));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n=== a(a|b) via DFA ===\n")).ToVerbatimString(false));
      _IDFA _17_dfa2;
      _IDFA _out11;
      _out11 = __default.Compile(_16_expr2, _4_alpha);
      _17_dfa2 = _out11;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("DFA states: ")).ToVerbatimString(false));
      Dafny.Helpers.Print(((_17_dfa2).dtor_nStates));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _18_d6;
      bool _out12;
      _out12 = __default.DFAMatch(_17_dfa2, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("ab"), _4_alpha);
      _18_d6 = _out12;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("a(a|b)  \"ab\" => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_18_d6));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _19_d7;
      bool _out13;
      _out13 = __default.DFAMatch(_17_dfa2, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("ba"), _4_alpha);
      _19_d7 = _out13;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("a(a|b)  \"ba\" => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_19_d7));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _20_d8;
      bool _out14;
      _out14 = __default.DFAMatch(_17_dfa2, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("aa"), _4_alpha);
      _20_d8 = _out14;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("a(a|b)  \"aa\" => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_20_d8));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
      bool _21_d9;
      bool _out15;
      _out15 = __default.DFAMatch(_17_dfa2, Dafny.Sequence<Dafny.Rune>.UnicodeFromString("a"), _4_alpha);
      _21_d9 = _out15;
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("a(a|b)  \"a\"  => ")).ToVerbatimString(false));
      Dafny.Helpers.Print((_21_d9));
      Dafny.Helpers.Print((Dafny.Sequence<Dafny.Rune>.UnicodeFromString("\n")).ToVerbatimString(false));
    }
    public static bool DFAMatch(_IDFA dfa, Dafny.ISequence<Dafny.Rune> s, Dafny.ISet<Dafny.Rune> alphabet)
    {
      bool accepts = false;
      BigInteger _0_state;
      _0_state = (dfa).dtor_start;
      BigInteger _hi0 = new BigInteger((s).Count);
      for (BigInteger _1_i = BigInteger.Zero; _1_i < _hi0; _1_i++) {
        _0_state = Dafny.Map<_System._ITuple2<BigInteger, Dafny.Rune>, BigInteger>.Select((dfa).dtor_trans,_System.Tuple2<BigInteger, Dafny.Rune>.create(_0_state, (s).Select(_1_i)));
      }
      accepts = ((dfa).dtor_accepting).Contains(_0_state);
      return accepts;
    }
    public static Dafny.ISequence<Dafny.Rune> SetToSeq(Dafny.ISet<Dafny.Rune> s)
    {
      Dafny.ISequence<Dafny.Rune> sq = Dafny.Sequence<Dafny.Rune>.Empty;
      sq = Dafny.Sequence<Dafny.Rune>.FromElements();
      Dafny.ISet<Dafny.Rune> _0_remaining;
      _0_remaining = s;
      while ((new BigInteger((_0_remaining).Count)).Sign == 1) {
        Dafny.Rune _1_c;
        foreach (Dafny.Rune _assign_such_that_0 in (_0_remaining).Elements) {
          _1_c = (Dafny.Rune)_assign_such_that_0;
          if ((_0_remaining).Contains(_1_c)) {
            goto after__ASSIGN_SUCH_THAT_0;
          }
        }
        throw new System.Exception("assign-such-that search produced no value");
      after__ASSIGN_SUCH_THAT_0: ;
        sq = Dafny.Sequence<Dafny.Rune>.Concat(sq, Dafny.Sequence<Dafny.Rune>.FromElements(_1_c));
        _0_remaining = Dafny.Set<Dafny.Rune>.Difference(_0_remaining, Dafny.Set<Dafny.Rune>.FromElements(_1_c));
      }
      return sq;
    }
    public static _IDFA Compile(_IExp<Dafny.Rune> e, Dafny.ISet<Dafny.Rune> alphabet)
    {
      _IDFA dfa = DFA.Default();
      Dafny.ISequence<Dafny.Rune> _0_alphaSeq;
      Dafny.ISequence<Dafny.Rune> _out0;
      _out0 = __default.SetToSeq(alphabet);
      _0_alphaSeq = _out0;
      _IExp<Dafny.Rune> _1_startExpr;
      _1_startExpr = __default.Normalize<Dafny.Rune>(e);
      Dafny.ISequence<_IExp<Dafny.Rune>> _2_states;
      _2_states = Dafny.Sequence<_IExp<Dafny.Rune>>.FromElements(_1_startExpr);
      Dafny.IMap<_IExp<Dafny.Rune>,BigInteger> _3_stateOf;
      _3_stateOf = Dafny.Map<_IExp<Dafny.Rune>, BigInteger>.FromElements(new Dafny.Pair<_IExp<Dafny.Rune>, BigInteger>(_1_startExpr, BigInteger.Zero));
      Dafny.IMap<_System._ITuple2<BigInteger, Dafny.Rune>,BigInteger> _4_trans;
      _4_trans = Dafny.Map<_System._ITuple2<BigInteger, Dafny.Rune>, BigInteger>.FromElements();
      Dafny.ISequence<bool> _5_expanded;
      _5_expanded = Dafny.Sequence<bool>.FromElements(false);
      bool _6_done;
      _6_done = false;
      while (!(_6_done)) {
        BigInteger _7_found;
        _7_found = new BigInteger(-1);
        BigInteger _hi0 = new BigInteger((_5_expanded).Count);
        for (BigInteger _8_i = BigInteger.Zero; _8_i < _hi0; _8_i++) {
          if (!((_5_expanded).Select(_8_i))) {
            _7_found = _8_i;
            goto after_0;
          }
        continue_0: ;
      ...
      after_0: ;
        if ((_7_found) == (new BigInteger(-1))) {
          _6_done = true;
        } else {
          BigInteger _9_sid;
          _9_sid = _7_found;
          _IExp<Dafny.Rune> _10_expr;
          _10_expr = (_2_states).Select(_9_sid);
          BigInteger _hi1 = new BigInteger((_0_alphaSeq).Count);
          for (BigInteger _11_ci = BigInteger.Zero; _11_ci < _hi1; _11_ci++) {
            Dafny.Rune _12_c;
            _12_c = (_0_alphaSeq).Select(_11_ci);
            _IExp<Dafny.Rune> _13_next;
            _13_next = __default.Normalize<Dafny.Rune>(Dafny.Helpers.Id<Func<Dafny.Rune, _IExp<Dafny.Rune>>>(__default.Delta<Dafny.Rune>(_10_expr))(_12_c));
            if (!(_3_stateOf).Contains(_13_next)) {
              BigInteger _14_nextId;
              _14_nextId = new BigInteger((_2_states).Count);
              _3_stateOf = Dafny.Map<_IExp<Dafny.Rune>, BigInteger>.Update(_3_stateOf, _13_next, _14_nextId);
              _2_states = Dafny.Sequence<_IExp<Dafny.Rune>>.Concat(_2_states, Dafny.Sequence<_IExp<Dafny.Rune>>.FromElements(_13_next));
              _5_expanded = Dafny.Sequence<bool>.Concat(_5_expanded, Dafny.Sequence<bool>.FromElements(false));
            }
            BigInteger _15_nextId;
            _15_nextId = Dafny.Map<_IExp<Dafny.Rune>, BigInteger>.Select(_3_stateOf,_13_next);
            _4_trans = Dafny.Map<_System._ITuple2<BigInteger, Dafny.Rune>, BigInteger>.Update(_4_trans, _System.Tuple2<BigInteger, Dafny.Rune>.create(_9_sid, _12_c), _15_nextId);
          }
          _5_expanded = Dafny.Sequence<bool>.Update(_5_expanded, _9_sid, true);
        }
      }
      Dafny.ISet<BigInteger> _16_accepting;
      _16_accepting = Dafny.Set<BigInteger>.FromElements();
      BigInteger _hi2 = new BigInteger((_2_states).Count);
      for (BigInteger _17_i = BigInteger.Zero; _17_i < _hi2; _17_i++) {
        if (__default.Eps<Dafny.Rune>((_2_states).Select(_17_i))) {
          _16_accepting = Dafny.Set<BigInteger>.Union(_16_accepting, Dafny.Set<BigInteger>.FromElements(_17_i));
        }
      }
      dfa = _module.DFA.create(new BigInteger((_2_states).Count), BigInteger.Zero, _16_accepting, _4_trans);
      return dfa;
    }
    public static _IExp<__A> FoldDelta<__A>(_IExp<__A> e, Dafny.ISequence<__A> s)
    {
    TAIL_CALL_START: ;
      if ((new BigInteger((s).Count)).Sign == 0) {
        return e;
      } else {
        _IExp<__A> _in0 = Dafny.Helpers.Id<Func<__A, _IExp<__A>>>(__default.Delta<__A>(e))((s).Select(BigInteger.Zero));
        Dafny.ISequence<__A> _in1 = (s).Drop(BigInteger.One);
        e = _in0;
        s = _in1;
        goto TAIL_CALL_START;
      }
    }
    public static Languages._ILang<__A> Denotational<__A>(_IExp<__A> e) {
      _IExp<__A> _source0 = e;
      {
        if (_source0.is_Zero) {
          return Languages.__default.Zero<__A>();
        }
      ...
      {
        if (_source0.is_One) {
          return Languages.__default.One<__A>();
        }
      }
      {
        if (_source0.is_Char) {
          __A _0_a = _source0.dtor_Char_a0;
          return Languages.__default.Singleton<__A>(_0_a);
        }
      }
      {
        if (_source0.is_Plus) {
          _IExp<__A> _1_e1 = _source0.dtor_Plus_a0;
          _IExp<__A> _2_e2 = _source0.dtor_Plus_a1;
          return Languages.__default.Plus<__A>(__default.Denotational<__A>(_1_e1), __default.Denotational<__A>(_2_e2));
        }
      }
      {
        if (_source0.is_Comp) {
          _IExp<__A> _3_e1 = _source0.dtor_Comp_a0;
          _IExp<__A> _4_e2 = _source0.dtor_Comp_a1;
          return Languages.__default.Comp<__A>(__default.Denotational<__A>(_3_e1), __default.Denotational<__A>(_4_e2));
        }
      }
      {
        _IExp<__A> _5_e1 = _source0.dtor_Star_a0;
        return Languages.__default.Star<__A>(__default.Denotational<__A>(_5_e1));
      }
    }
    public static bool Eps<__A>(_IExp<__A> e) {
      _IExp<__A> _source0 = e;
      {
        if (_source0.is_Zero) {
          return false;
      ...
      }
      {
        if (_source0.is_Plus) {
          _IExp<__A> _1_e1 = _source0.dtor_Plus_a0;
          _IExp<__A> _2_e2 = _source0.dtor_Plus_a1;
          return (__default.Eps<__A>(_1_e1)) || (__default.Eps<__A>(_2_e2));
        }
      }
      {
        if (_source0.is_Comp) {
          _IExp<__A> _3_e1 = _source0.dtor_Comp_a0;
          _IExp<__A> _4_e2 = _source0.dtor_Comp_a1;
          return (__default.Eps<__A>(_3_e1)) && (__default.Eps<__A>(_4_e2));
        }
      }
      {
        _IExp<__A> _5_e1 = _source0.dtor_Star_a0;
    ...
    public static Func<__A, _IExp<__A>> Delta<__A>(_IExp<__A> e) {
      return Dafny.Helpers.Id<Func<_IExp<__A>, Func<__A, _IExp<__A>>>>((_0_e) => ((System.Func<__A, _IExp<__A>>)((_1_a) => {
        return ((System.Func<_IExp<__A>>)(() => {
          _IExp<__A> _source0 = _0_e;
          {
            if (_source0.is_Zero) {
              return _module.Exp<__A>.create_Zero();
            }
          }
          {
            if (_source0.is_One) {
          ...
          {
            if (_source0.is_Char) {
              __A _2_b = _source0.dtor_Char_a0;
              if (object.Equals(_1_a, _2_b)) {
                return _module.Exp<__A>.create_One();
              } else {
                return _module.Exp<__A>.create_Zero();
              }
            }
          ...
          {
            if (_source0.is_Plus) {
              _IExp<__A> _3_e1 = _source0.dtor_Plus_a0;
              _IExp<__A> _4_e2 = _source0.dtor_Plus_a1;
              return _module.Exp<__A>.create_Plus(Dafny.Helpers.Id<Func<__A, _IExp<__A>>>(__default.Delta<__A>(_3_e1))(_1_a), Dafny.Helpers.Id<Func<__A, _IExp<__A>>>(__default.Delta<__A>(_4_e2))(_1_a));
            }
          }
          {
            if (_source0.is_Comp) {
              _IExp<__A> _5_e1 = _source0.dtor_Comp_a0;
              _IExp<__A> _6_e2 = _source0.dtor_Comp_a1;
              return _module.Exp<__A>.create_Plus(_module.Exp<__A>.create_Comp(Dafny.Helpers.Id<Func<__A, _IExp<__A>>>(__default.Delta<__A>(_5_e1))(_1_a), _6_e2), _module.Exp<__A>.create_Comp(((__default.Eps<__A>(_5_e1)) ? (_module.Exp<__A>.create_One()) : (_module.Exp<__A>.create_Zero())), Dafny.Helpers.Id<Func<__A, _IExp<__A>>>(__default.Delta<__A>(_6_e2))(_1_a)));
            }
          }
          {
            _IExp<__A> _7_e1 = _source0.dtor_Star_a0;
            return _module.Exp<__A>.create_Comp(Dafny.Helpers.Id<Func<__A, _IExp<__A>>>(__default.Delta<__A>(_7_e1))(_1_a), _module.Exp<__A>.create_Star(_7_e1));
          }
        }))();
      })))(e);
    }
    public static Languages._ILang<__A> Operational<__A>(_IExp<__A> e) {
      return new Languages.Lang__Lazy<__A>(() => { return new Languages.Lang_Alpha<__A>(__default.Eps<__A>(e), Dafny.Helpers.Id<Func<_IExp<__A>, Func<__A, Languages._ILang<__A>>>>((_0_e) => ((System.Func<__A, Languages._ILang<__A>>)((_1_a) => {
  return __default.Operational<__A>(Dafny.Helpers.Id<Func<__A, _IExp<__A>>>(__default.Delta<__A>(_0_e))(_1_a));
})))(e)); });
    }
    public static _IExp<__A> Normalize<__A>(_IExp<__A> e) {
      _IExp<__A> _source0 = e;
      {
        if (_source0.is_Zero) {
          return _module.Exp<__A>.create_Zero();
      ...
      }
      {
        if (_source0.is_Char) {
          __A _0_a = _source0.dtor_Char_a0;
          return _module.Exp<__A>.create_Char(_0_a);
        }
      }
      {
        if (_source0.is_Plus) {
          _IExp<__A> _1_e1 = _source0.dtor_Plus_a0;
          _IExp<__A> _2_e2 = _source0.dtor_Plus_a1;
          return __default.NormPlus<__A>(__default.Normalize<__A>(_1_e1), __default.Normalize<__A>(_2_e2));
        }
      }
      {
        if (_source0.is_Comp) {
          _IExp<__A> _3_e1 = _source0.dtor_Comp_a0;
          _IExp<__A> _4_e2 = _source0.dtor_Comp_a1;
          return __default.NormComp<__A>(__default.Normalize<__A>(_3_e1), __default.Normalize<__A>(_4_e2));
        }
      }
      {
        _IExp<__A> _5_e1 = _source0.dtor_Star_a0;
        return __default.NormStar<__A>(__default.Normalize<__A>(_5_e1));
      }
    }
    public static _IExp<__A> NormPlus<__A>(_IExp<__A> e1, _IExp<__A> e2)
    {
      if (object.Equals(e1, _module.Exp<__A>.create_Zero())) {
        return e2;
      } else if (object.Equals(e2, _module.Exp<__A>.create_Zero())) {
        return e1;
      } else if (object.Equals(e1, e2)) {
        return e1;
      } else {
        return _module.Exp<__A>.create_Plus(e1, e2);
      }
    }
    public static _IExp<__A> NormComp<__A>(_IExp<__A> e1, _IExp<__A> e2)
    {
      if (object.Equals(e1, _module.Exp<__A>.create_Zero())) {
        return _module.Exp<__A>.create_Zero();
      } else if (object.Equals(e2, _module.Exp<__A>.create_Zero())) {
        return _module.Exp<__A>.create_Zero();
      } else if (object.Equals(e1, _module.Exp<__A>.create_One())) {
        return e2;
      } else {
        return _module.Exp<__A>.create_Comp(e1, e2);
      }
    }
    public static _IExp<__A> NormStar<__A>(_IExp<__A> e) {
      _IExp<__A> _source0 = e;
      {
        if (_source0.is_Zero) {
          return _module.Exp<__A>.create_One();
      ...
      {
        return _module.Exp<__A>.create_Star(e);
      }
    }
    public static Func<__A, _IExp<__A>> NDelta<__A>(_IExp<__A> e) {
      return Dafny.Helpers.Id<Func<_IExp<__A>, Func<__A, _IExp<__A>>>>((_0_e) => ((System.Func<__A, _IExp<__A>>)((_1_a) => {
        return __default.Normalize<__A>(Dafny.Helpers.Id<Func<__A, _IExp<__A>>>(__default.Delta<__A>(_0_e))(_1_a));
      })))(e);
    }
    public static _IExp<__A> FoldNDelta<__A>(_IExp<__A> e, Dafny.ISequence<__A> s)
    {
    TAIL_CALL_START: ;
      if ((new BigInteger((s).Count)).Sign == 0) {
        return e;
      } else {
        _IExp<__A> _in0 = Dafny.Helpers.Id<Func<__A, _IExp<__A>>>(__default.NDelta<__A>(e))((s).Select(BigInteger.Zero));
        Dafny.ISequence<__A> _in1 = (s).Drop(BigInteger.One);
        e = _in0;
        s = _in1;
        goto TAIL_CALL_START;
    ...
    public static bool Match(_IExp<Dafny.Rune> e, Dafny.ISequence<Dafny.Rune> s)
    {
      bool accepts = false;
      _IExp<Dafny.Rune> _0_current;
      _0_current = e;
      BigInteger _hi0 = new BigInteger((s).Count);
      for (BigInteger _1_i = BigInteger.Zero; _1_i < _hi0; _1_i++) {
        _0_current = Dafny.Helpers.Id<Func<Dafny.Rune, _IExp<Dafny.Rune>>>(__default.Delta<Dafny.Rune>(_0_current))((s).Select(_1_i));
      }
      accepts = __default.Eps<Dafny.Rune>(_0_current);
      return accepts;
    }
  }

  public interface _IDFA {
    bool is_DFA { get; }
    BigInteger dtor_nStates { get; }
    BigInteger dtor_start { get; }
    Dafny.ISet<BigInteger> dtor_accepting { get; }
    Dafny.IMap<_System._ITuple2<BigInteger, Dafny.Rune>,BigInteger> dtor_trans { get; }
    _IDFA DowncastClone();
  }
  public class DFA : _IDFA {
    public readonly BigInteger _nStates;
    public readonly BigInteger _start;
    public readonly Dafny.ISet<BigInteger> _accepting;
    public readonly Dafny.IMap<_System._ITuple2<BigInteger, Dafny.Rune>,BigInteger> _trans;
    public DFA(BigInteger nStates, BigInteger start, Dafny.ISet<BigInteger> accepting, Dafny.IMap<_System._ITuple2<BigInteger, Dafny.Rune>,BigInteger> trans) {
      this._nStates = nStates;
      this._start = start;
      this._accepting = accepting;
      this._trans = trans;
    }
    public _IDFA DowncastClone() {
      if (this is _IDFA dt) { return dt; }
      return new DFA(_nStates, _start, _accepting, _trans);
    }
    public override bool Equals(object other) {
      var oth = other as DFA;
      return oth != null && this._nStates == oth._nStates && this._start == oth._start && object.Equals(this._accepting, oth._accepting) && object.Equals(this._trans, oth._trans);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 0;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._nStates));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._start));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this._accepting));
    ...
    public override string ToString() {
      string s = "DFA.DFA";
      s += "(";
      s += Dafny.Helpers.ToString(this._nStates);
      s += ", ";
      s += Dafny.Helpers.ToString(this._start);
      s += ", ";
      s += Dafny.Helpers.ToString(this._accepting);
      s += ", ";
      s += Dafny.Helpers.ToString(this._trans);
      s += ")";
      return s;
    }
    private static readonly _IDFA theDefault = create(BigInteger.Zero, BigInteger.Zero, Dafny.Set<BigInteger>.Empty, Dafny.Map<_System._ITuple2<BigInteger, Dafny.Rune>, BigInteger>.Empty);
    public static _IDFA Default() {
      return theDefault;
    }
    private static readonly Dafny.TypeDescriptor<_IDFA> _TYPE = new Dafny.TypeDescriptor<_IDFA>(DFA.Default());
    public static Dafny.TypeDescriptor<_IDFA> _TypeDescriptor() {
      return _TYPE;
    }
    public static _IDFA create(BigInteger nStates, BigInteger start, Dafny.ISet<BigInteger> accepting, Dafny.IMap<_System._ITuple2<BigInteger, Dafny.Rune>,BigInteger> trans) {
      return new DFA(nStates, start, accepting, trans);
    }
    public static _IDFA create_DFA(BigInteger nStates, BigInteger start, Dafny.ISet<BigInteger> accepting, Dafny.IMap<_System._ITuple2<BigInteger, Dafny.Rune>,BigInteger> trans) {
      return create(nStates, start, accepting, trans);
    }
    public bool is_DFA { get { return true; } }
    public BigInteger dtor_nStates {
      get {
    ...
    public Dafny.ISet<BigInteger> dtor_accepting {
      get {
        return this._accepting;
      }
    }
    public Dafny.IMap<_System._ITuple2<BigInteger, Dafny.Rune>,BigInteger> dtor_trans {
      get {
        return this._trans;
      }
    }
  ...
  public interface _IExp<A> {
    ...
    bool is_Star { get; }
    A dtor_Char_a0 { get; }
    _IExp<A> dtor_Plus_a0 { get; }
    _IExp<A> dtor_Plus_a1 { get; }
    _IExp<A> dtor_Comp_a0 { get; }
    _IExp<A> dtor_Comp_a1 { get; }
    _IExp<A> dtor_Star_a0 { get; }
    _IExp<__A> DowncastClone<__A>(Func<A, __A> converter0);
  }
  public abstract class Exp<A> : _IExp<A> {
    public Exp() {
    }
    public static _IExp<A> Default() {
      return create_Zero();
    }
    public static Dafny.TypeDescriptor<_IExp<A>> _TypeDescriptor() {
      return new Dafny.TypeDescriptor<_IExp<A>>(Exp<A>.Default());
    }
    public static _IExp<A> create_Zero() {
      return new Exp_Zero<A>();
    }
    public static _IExp<A> create_One() {
      return new Exp_One<A>();
    }
    public static _IExp<A> create_Char(A _a0) {
      return new Exp_Char<A>(_a0);
    }
    public static _IExp<A> create_Plus(_IExp<A> _a0, _IExp<A> _a1) {
      return new Exp_Plus<A>(_a0, _a1);
    }
    public static _IExp<A> create_Comp(_IExp<A> _a0, _IExp<A> _a1) {
      return new Exp_Comp<A>(_a0, _a1);
    }
    public static _IExp<A> create_Star(_IExp<A> _a0) {
      return new Exp_Star<A>(_a0);
    }
    public bool is_Zero { get { return this is Exp_Zero<A>; } }
    public bool is_One { get { return this is Exp_One<A>; } }
    public bool is_Char { get { return this is Exp_Char<A>; } }
    public bool is_Plus { get { return this is Exp_Plus<A>; } }
    public bool is_Comp { get { return this is Exp_Comp<A>; } }
    public bool is_Star { get { return this is Exp_Star<A>; } }
    public A dtor_Char_a0 {
      get {
        var d = this;
        return ((Exp_Char<A>)d)._a0;
      }
    }
    public _IExp<A> dtor_Plus_a0 {
      get {
        var d = this;
        return ((Exp_Plus<A>)d)._a0;
      }
    }
    public _IExp<A> dtor_Plus_a1 {
      get {
        var d = this;
        return ((Exp_Plus<A>)d)._a1;
      }
    }
    public _IExp<A> dtor_Comp_a0 {
      get {
        var d = this;
        return ((Exp_Comp<A>)d)._a0;
      }
    }
    public _IExp<A> dtor_Comp_a1 {
      get {
        var d = this;
        return ((Exp_Comp<A>)d)._a1;
      }
    }
    public _IExp<A> dtor_Star_a0 {
      get {
        var d = this;
        return ((Exp_Star<A>)d)._a0;
      }
    }
    public abstract _IExp<__A> DowncastClone<__A>(Func<A, __A> converter0);
  }
  public class Exp_Zero<A> : Exp<A> {
    public Exp_Zero() : base() {
    }
    public override _IExp<__A> DowncastClone<__A>(Func<A, __A> converter0) {
      if (this is _IExp<__A> dt) { return dt; }
      return new Exp_Zero<__A>();
    }
    public override bool Equals(object other) {
      var oth = other as Exp_Zero<A>;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
  ...
  public class Exp_One<A> : Exp<A> {
    public Exp_One() : base() {
    }
    public override _IExp<__A> DowncastClone<__A>(Func<A, __A> converter0) {
      if (this is _IExp<__A> dt) { return dt; }
      return new Exp_One<__A>();
    }
    public override bool Equals(object other) {
      var oth = other as Exp_One<A>;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
  ...
  public class Exp_Char<A> : Exp<A> {
    public readonly A _a0;
    public Exp_Char(A _a0) : base() {
      this._a0 = _a0;
    }
    public override _IExp<__A> DowncastClone<__A>(Func<A, __A> converter0) {
      if (this is _IExp<__A> dt) { return dt; }
      return new Exp_Char<__A>(converter0(_a0));
    }
    public override bool Equals(object other) {
      var oth = other as Exp_Char<A>;
  ...
  public class Exp_Plus<A> : Exp<A> {
    public readonly _IExp<A> _a0;
    public readonly _IExp<A> _a1;
    public Exp_Plus(_IExp<A> _a0, _IExp<A> _a1) : base() {
      this._a0 = _a0;
      this._a1 = _a1;
    }
    public override _IExp<__A> DowncastClone<__A>(Func<A, __A> converter0) {
      if (this is _IExp<__A> dt) { return dt; }
      return new Exp_Plus<__A>((_a0).DowncastClone<__A>(Dafny.Helpers.CastConverter<A, __A>), (_a1).DowncastClone<__A>(Dafny.Helpers.CastConverter<A, __A>));
    }
    public override bool Equals(object other) {
      var oth = other as Exp_Plus<A>;
  ...
  public class Exp_Comp<A> : Exp<A> {
    public readonly _IExp<A> _a0;
    public readonly _IExp<A> _a1;
    public Exp_Comp(_IExp<A> _a0, _IExp<A> _a1) : base() {
      this._a0 = _a0;
      this._a1 = _a1;
    }
    public override _IExp<__A> DowncastClone<__A>(Func<A, __A> converter0) {
      if (this is _IExp<__A> dt) { return dt; }
      return new Exp_Comp<__A>((_a0).DowncastClone<__A>(Dafny.Helpers.CastConverter<A, __A>), (_a1).DowncastClone<__A>(Dafny.Helpers.CastConverter<A, __A>));
    }
    public override bool Equals(object other) {
      var oth = other as Exp_Comp<A>;
      return oth != null && object.Equals(this._a0, oth._a0) && object.Equals(this._a1, oth._a1);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
  ...
  public class Exp_Star<A> : Exp<A> {
    public readonly _IExp<A> _a0;
    public Exp_Star(_IExp<A> _a0) : base() {
      this._a0 = _a0;
    }
    public override _IExp<__A> DowncastClone<__A>(Func<A, __A> converter0) {
      if (this is _IExp<__A> dt) { return dt; }
      return new Exp_Star<__A>((_a0).DowncastClone<__A>(Dafny.Helpers.CastConverter<A, __A>));
    }
    public override bool Equals(object other) {
      var oth = other as Exp_Star<A>;
      return oth != null && object.Equals(this._a0, oth._a0);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
  ...
  public interface _ILang<A> {
    ...
    bool is_Alpha { get; }
    bool dtor_eps { get; }
    Func<A, _ILang<A>> dtor_delta { get; }
    _ILang<__A> DowncastClone<__A>(Func<A, __A> converter0);
  }
  public abstract class Lang<A> : _ILang<A> {
    public Lang() {
    }
    public static _ILang<A> Default(A _default_A) {
      return new Lang__Lazy<A>(() => { return create_Alpha(false, ((A x1) => Lang<A>.Default(_default_A))); });
    }
    public static Dafny.TypeDescriptor<_ILang<A>> _TypeDescriptor(Dafny.TypeDescriptor<A> _td_A) {
      return new Dafny.TypeDescriptor<_ILang<A>>(Lang<A>.Default(_td_A.Default()));
    }
    public abstract _ILang<A> _Get();
    public static _ILang<A> create_Alpha(bool eps, Func<A, _ILang<A>> delta) {
      return new Lang_Alpha<A>(eps, delta);
    }
    public bool is_Alpha { get { return this is Lang_Alpha<A>; } }
    public bool dtor_eps {
    ...
    public Func<A, _ILang<A>> dtor_delta {
      get {
        var d = this._Get();
        return ((Lang_Alpha<A>)d)._delta;
      }
  ...
  public class Lang__Lazy<A> : Lang<A> {
    ...
    public override _ILang<__A> DowncastClone<__A>(Func<A, __A> converter0) {
      if (this is _ILang<__A> dt) { return dt; }
      return new Lang__Lazy<__A>(() => _Get().DowncastClone<__A>(converter0));
    }
    public override _ILang<A> _Get() { if (c != null) { d = c(); c = null; } return d; }
    public override string ToString() { return _Get().ToString(); }
  }
  public class Lang_Alpha<A> : Lang<A> {
    public readonly bool _eps;
    public readonly Func<A, _ILang<A>> _delta;
    public Lang_Alpha(bool eps, Func<A, _ILang<A>> delta) : base() {
      this._eps = eps;
      this._delta = delta;
    }
    public override _ILang<A> _Get() { return this; }
    public override _ILang<__A> DowncastClone<__A>(Func<A, __A> converter0) {
      if (this is _ILang<__A> dt) { return dt; }
      return new Lang_Alpha<__A>(_eps, (_delta).DowncastClone<A, _ILang<A>, __A, _ILang<__A>>(Dafny.Helpers.CastConverter<__A, A>, Dafny.Helpers.CastConverter<_ILang<A>, _ILang<__A>>));
    }
    public override bool Equals(object other) {
      var oth = other as Lang_Alpha<A>;
      return oth != null && this._eps == oth._eps && object.Equals(this._delta, oth._delta);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
